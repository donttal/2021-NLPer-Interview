...
dropout一大缺点就是**代价函数J不再被明确定义**，每次迭代，都会随机移除一些节点，如果再三检查梯度下降的性能，实际上是很难进行复查的。定义明确的代价函数J每次迭代后都会下降，因为我们所优化的代价函数J实际上并没有明确定义，或者说在某种程度上很难计算，所以我们失去了调试工具来绘制这样的图片。
...
### Dropout加在哪里
4. word embedding层后、pooling层后、FC层（全联接层） 
