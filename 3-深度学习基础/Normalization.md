...
tags: 深度学习
[参考网址](https://blog.csdn.net/shwan_ma/article/details/85292024)
...
![](https://tva1.sinaimg.cn/large/008eGmZEgy1go32jn8d87j30tm0hqjxd.jpg)

归一化后，寻求最优解的过程为：

![](https://tva1.sinaimg.cn/large/008eGmZEgy1go32kjkan7j30x40gq0yn.jpg)
...
---
Normalization（WN)是在权值的维度上做的归一化。WN的做法是将权值向量 [公式] 在其欧氏范数和其方向上解耦成了参数向量 [公式] 和参数标量 [公式] 后使用SGD分别优化这两个参数。
...
其中w是k维权重向量，b是标量偏差，x是k维输入特征，$ϕ ( . )$是激活函数。

WN的重参数表示：对权重w 用参数向量v和标量g进行表示，则新参数表示为：
$${w}=\frac{g}{\|{v}\|}{v}
$$


其中v是k维向量，g是标量，∥v∥为v的欧式范数。我们注意到，此时w则被重参数为v和g两个参数。
...
### 文本模型使用LN较多
原因文本处理不定长，BN是按照bitch数量进行归一化的过程，长序列参照的信息比较少
